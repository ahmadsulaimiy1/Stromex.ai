...
# --- 6. An import of our own code that does not exist --------------------------------------------
#
# The fourth class of failure CI has caught, and the most self-inflicted: deleting two unused
# composables by cutting from a marker to end-of-file also deleted a third one *below* the marker
# that was in active use, leaving an import pointing at nothing. The shape check had a rule for a
# symbol used without an import and no rule for the exact inverse.
#
# So every `import ai.sautiy.…` must resolve to a declaration somewhere in the source tree. This
# cannot see the classpath, so it only checks our own packages — which is where the mistake was.

python3 - "$ROOT" <<'PYTHON' || STATUS=1
import os, re, sys

roots = [sys.argv[1], 'apps/sautiy/sautiy-core/src/main/kotlin']
declared = set()
sources = []
for root in roots:
    for directory, _, files in os.walk(root):
        for name in files:
            if name.endswith('.kt'):
                ...
                sources.append(path)
                text = open(path, encoding='utf-8').read()
                # `fun interface` first, or `interface` is read as the name. Extension receivers
                # are stripped, or `fun Modifier.pressable` declares "Modifier" instead.
                for match in re.finditer(
                    r'\b(?:fun\s+interface|data\s+class|value\s+class|enum\s+class'
                    r'|annotation\s+class|class|object|interface|typealias|fun|val|var)\s+'
...
for path in sources:
    for line in open(path, encoding='utf-8'):
        match = re.match(r'import\s+(ai\.sautiy\.[\w.]+)', line.strip())
        if not match:
            continue
        symbol = match.group(1).rsplit('.', 1)[-1]
        # Lower-case leaf is an extension function or a property; those are matched by `fun`/`val`
        # above. A leaf that is not declared anywhere in our own source cannot resolve.
        # R and BuildConfig are generated by the Android build, not written by anybody.
        if symbol in ('R', 'BuildConfig', '*'):
            continue
        if symbol not in declared:
            missing.append(f'{path}: imports {match.group(1)}, which nothing declares')

if missing:
    print('error: an import points at one of our own symbols that does not exist.')
